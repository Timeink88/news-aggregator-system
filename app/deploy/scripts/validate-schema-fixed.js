...
class SchemaValidator {
   ...
   * 判断是否在SQL插入语句的字符串中
   */
  isInSQLInsertString(line, pos) {
    // 简单检查是否在INSERT语句的字符串中
    const insertMatch = line.match(/INSERT.*VALUES.*'/i);
  ...
  validateSyntax(sqlContent) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 只有在不在字符串中时才检查关键字
      if (!inString) {
        // 检查关键字拼写 - 仅在CREATE语句中检查
        if (line.trim().startsWith('CREATE')) {
          const sqlKeywords = ['INDEX', 'ON', 'CHECK'];
          for (const keyword of sqlKeywords) {
            ...
            if (matches && matches.length > 0) {
              ...
              if (matches.some(m => m !== m.toUpperCase())) {
                ...
                this.warnings.push({
                  line: lineNum,
                  message: `SQL关键字 ${keyword} 的大小写不一致，建议使用大写 (当前: ${lowercaseMatches.join(', ')})`
                });
              }
            }
          }
  ...
  validateFunctions(sqlContent) {
    ...
    if (securityDefinerCount === 0) {
      ...
      console.log(`✅ 使用 ${securityDefinerCount} 个SECURITY DEFINER函数`);
    }
